...
//   1. Several overlapping passes at different sizes/rotations/weights — a heavy
//      bold outer loop plus lighter inner passes — so you see multiple loop
//      lines like a real multi-pass scribble.
//   2. A spiral connector + an overshoot tail so the loop doesn't tidily close.
//   3. An SVG roughen filter (feTurbulence -> feDisplacementMap) warping every
//      stroke with organic, pen-on-paper jitter.
//
...
const SketchCircle = () => (
  <svg
    ...
    className="pointer-events-none absolute -left-[0.85em] -right-[0.85em] -top-[0.82em] -bottom-[0.82em] overflow-visible sm:-left-[0.78em] sm:-right-[0.78em] sm:-top-[0.55em] sm:-bottom-[0.95em]"
  >
    <defs>
    ...
    >
      ...
      <ellipse
        ...
        opacity="0.6"
      />
      {/* Spiral connector — a diagonal sweep across the lower-left so the passes
          read as one continuous looping motion. */}
      <path
        d="M30 120 C 58 78, 112 52, 156 45"
        strokeWidth="2.6"
        opacity="0.7"
      />
      {/* Overshoot tail — the marker hooking out past the loop at lower-left. */}
      <path
        d="M44 138 C 16 150, 20 168, 52 162"
        strokeWidth="4.4"
        opacity="0.88"
      />
    </g>
