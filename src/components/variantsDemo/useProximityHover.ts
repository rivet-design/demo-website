// Verbatim port of rivet/src/ui/src/hooks/use-proximity-hover.ts — the hook
// behind the variants panel's "fluid" hover: a single overlay tracks whichever
// row the cursor is nearest, measured with offset* so it's transform-safe.
import {
  useRef,
  useState,
  useCallback,
...
export function useProximityHover<T extends HTMLElement>(
  ...
  const measureItems = useCallback(() => {
    ...
    itemsRef.current.forEach((element, index) => {
      ...
      // relative to the offsetParent (the scroll container).
      rects[index] = {
  ...
  const registerItem = useCallback(
    (index: number, element: HTMLElement | null) => {
      ...
      // Coalesce rapid register/unregister calls into a single remeasure on the
      // next frame.
