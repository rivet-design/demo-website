...
const Connectors = () => {
  ...
  return (
    <section className="w-full max-w-3xl rounded-lg border border-border bg-secondary p-6 shadow-sm">
      {/* Header */}
      <header className="mb-5 flex items-start justify-between gap-4">
        <div>
          <h2 className="type-heading-2 text-accent-foreground">Connect your sources</h2>
          <p className="type-caption mt-1 text-muted-foreground">
            Bring references into Rivet from the places you already collect them.
          </p>
        </div>
        <span className="type-overline shrink-0 rounded-full bg-main px-2.5 py-1 text-muted-foreground">
      ...
      <ul className="grid grid-cols-1 gap-4 sm:grid-cols-3">
        {SOURCES.map((source) => {
          ...
          return (
            <li
              key={source.id}
              className="flex flex-col rounded-lg border border-border bg-main p-5 shadow-sm"
            >
              {/* Icon tile + connected badge */}
              <div className="flex items-start justify-between">
                <span
                  className={`flex h-14 w-14 shrink-0 items-center justify-center rounded-lg ${source.tile}`}
                >
                  {source.icon}
                </span>
                {isConnected && (
              ...
              <div className="mt-4 flex-1">
                <h3 className="type-label-lg text-accent-foreground">{source.name}</h3>
                <p className="type-caption mt-1 text-muted-foreground">
                  {source.description}
                </p>
                {isConnected && source.connectedDetail && (
                  <p className="type-caption mt-2 font-medium text-accent-foreground">
              ...
              <button
                ...
                className={
                  isConnected
                    ? 'type-label mt-5 w-full rounded-lg border border-border px-3.5 py-2 text-accent-foreground transition-colors hover:bg-secondary'
                    : 'type-label mt-5 w-full rounded-lg bg-primary px-3.5 py-2 text-white transition-colors hover:bg-primary-hover'
                }
              >
...
// Materialized Rivet variant "CardGrid" — route: /experiments/connectors/card-grid
...
export default CardGridPreview;
