  ...
  return (
    <section className="w-full max-w-xl overflow-hidden rounded-lg border border-border bg-main shadow-sm">
      ...
      <header className="flex items-center justify-between gap-4 border-b border-border px-5 py-3">
        <h2 className="type-label-lg text-accent-foreground">Connect your sources</h2>
        <span className="type-overline shrink-0 text-muted-foreground">
          {connectedCount} of {SOURCES.length} connected
        </span>
      </header>

      {/* Dense source rows — one line each, optimized for scanning */}
      <ul className="divide-y divide-border">
        {SOURCES.map((source) => {
          ...
          return (
            ...
            >
              <span
                className={`flex h-8 w-8 shrink-0 items-center justify-center rounded-md ${source.tile}`}
              >
                {source.icon}
              </span>

              <div className="flex min-w-0 flex-1 items-center gap-2">
                <span className="type-label shrink-0 text-accent-foreground">{source.name}</span>
                {isConnected ? (
                  <span className="inline-flex shrink-0 items-center gap-1 rounded-full bg-primary/10 px-1.5 py-0.5 text-[11px] font-medium leading-none text-primary">
                    <CheckIcon />
                    Connected
                  </span>
                ) : (
                  <span className="shrink-0 rounded-full bg-secondary px-1.5 py-0.5 text-[11px] font-medium leading-none text-muted-foreground">
                    Not connected
                  </span>
                )}
                <span aria-hidden className="hidden h-3 w-px shrink-0 bg-border sm:block" />
                <p className="hidden min-w-0 truncate type-caption text-muted-foreground sm:block">
              ...
              <button
                ...
                className={
                  isConnected
                    ? 'type-caption shrink-0 rounded-md border border-border px-2.5 py-1 font-medium text-accent-foreground transition-colors hover:bg-secondary'
                    : 'type-caption shrink-0 rounded-md bg-primary px-2.5 py-1 font-medium text-white transition-colors hover:bg-primary-hover'
                }
              >
...
// Materialized Rivet variant "CompactList" — route: /experiments/connectors/compact-list
...
export default CompactListPreview;
