...
const SOURCES: Source[] = [
  ...
  },
  {
    ...
    connectedDetail: '',
    tile: 'bg-accent-foreground text-white',
    icon: <ArenaGlyph />,
  },
  {
    id: 'files',
...
const Connectors = () => {
  ...
  const progress = Math.round((connectedCount / total) * 100);

  const SUCCESS = '#10b981';

  return (
    <section className="w-full max-w-xl overflow-hidden rounded-lg border border-border bg-main shadow-sm">
      {/* Header + progress */}
      <header className="border-b border-border px-6 py-5">
        <div className="flex items-start justify-between gap-4">
          <div>
            <span className="type-overline text-muted-foreground">Setup checklist</span>
            <h2 className="type-heading-2 mt-1 text-accent-foreground">Connect your sources</h2>
            <p className="type-caption mt-1 text-muted-foreground">
              Work through these steps to bring your references into Rivet.
            </p>
          </div>
          {/* Ring-style progress badge */}
          <div
            className="relative flex h-14 w-14 shrink-0 items-center justify-center rounded-full"
            style={{
              background: `conic-gradient(${allDone ? SUCCESS : '#E14017'} ${progress}%, var(--secondary, #f1f1f1) 0)`,
            }}
            aria-label={`${connectedCount} of ${total} steps complete`}
          >
            <span className="flex h-11 w-11 items-center justify-center rounded-full bg-main">
              {allDone ? (
                <span style={{ color: SUCCESS }}>
                  <CheckIcon />
                </span>
              ) : (
                <span className="type-label-lg text-accent-foreground">
                  {connectedCount}
        ...
        <div className="mt-4">
          <div className="h-2 w-full overflow-hidden rounded-full bg-secondary">
            <div
              ...
              style={{ width: `${progress}%`, backgroundColor: allDone ? SUCCESS : '#E14017' }}
            />
          </div>
          <p className="type-caption mt-2 text-muted-foreground">
            {allDone ? (
              ...
              `${total - connectedCount} step${total - connectedCount === 1 ? '' : 's'} left to finish setup.`
            )}
          </p>
        </div>
      </header>
      ...
      <ol className="px-3 py-2">
        {SOURCES.map((source, index) => {
          ...
          return (
            <li key={source.id} className="relative flex gap-4 px-3">
              {/* Step rail: marker + connecting line */}
              <div className="flex flex-col items-center">
                <span
                  ...
                  aria-label={isConnected ? 'Step complete' : `Step ${index + 1}`}
                >
                  {isConnected ? (
                    <CheckIcon />
                  ) : (
                    <span className="flex h-7 w-7 items-center justify-center rounded-full border border-border bg-secondary type-label text-muted-foreground">
                      {index + 1}
                    </span>
                  )}
                </span>
                {!isLast && (
                  <span
                    ...
                    style={{ backgroundColor: isConnected ? SUCCESS : 'var(--border, #e6e6e6)' }}
                  />
                )}
              </div>
              ...
              {/* Step card */}
              <div
                className={`mb-2 flex flex-1 items-center gap-4 rounded-lg px-4 py-3 transition-colors ${
                  isConnected ? 'border' : 'border border-transparent'
                }`}
                style={
                  isConnected
                    ? { backgroundColor: `${SUCCESS}0d`, borderColor: `${SUCCESS}33` }
                    : undefined
                }
              >
                {/* Source icon tile */}
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2">
                    <span className="type-label-lg text-accent-foreground">{source.name}</span>
                    {isConnected && (
                      <span
                ...
                <button
                  ...
                  className={
                    isConnected
                      ? 'type-label shrink-0 rounded-lg border border-border bg-main px-3.5 py-2 text-accent-foreground transition-colors hover:bg-secondary'
...
// Materialized Rivet variant "GuidedChecklist" — route: /experiments/connectors/guided-checklist
const GuidedChecklistPreview = () => (
  <main className="flex min-h-screen w-full items-center justify-center bg-secondary px-6 py-16">
    <Connectors />
  </main>
);

export default GuidedChecklistPreview;
